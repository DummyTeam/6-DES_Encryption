...
public:
	...
	static string formatMessage(string input) {
		...
		string totalMessage = Utils::stringToHex(input) + carriageReturn + lineFeed;
	...
	string runThroughSBoxes(string input48) {
		...
		for (size_t i = 0; i < 8; i++)
		{
			...
			int row = Utils::binToDec(string({ SInput[0], SInput[SInput.size() - 1] }));
			int column = Utils::binToDec(SInput.substr(1, 4));

			res += Utils::OR(Utils::decToBin(S[i][row][column]), "0000");
	...
	string f(string input, string key) {
		...
		string res48 = Utils::XOR(key, expanded48);
	...
	string encryptBlock(string mesBlock64, string keyBlock64) {
		...
		for (int i = 1; i < 17; i++)
		{
			...
			// Rn = Ln-1 + f(Rn-1,Kn)
			R[i] = Utils::XOR(L[i - 1], f(R[i - 1], K[i - 1]));
		}
	...
	string decryptBlock(string chipperBlock64, string keyBlock64) {
		...
		for (int i = 1; i < 17; i++)
		{
			...
			// Rn = Ln-1 + f(Rn-1,Kn)
			R[i] = Utils::XOR(L[i - 1], f(R[i - 1], K[15 - (i - 1)]));
		}
	...
	string encrypt(string binMessage, string binKey) {
		string encryptedMessageBin = "";

		for (int i = 0; i < binMessage.size() / 64; i++)
		{
			encryptedMessageBin += encryptBlock(binMessage.substr(i * 64, 64), binKey);
		}

		return encryptedMessageBin;
	}

	string decrypt(string encryptedMessageBin, string binKey) {
		string decryptedMessageBin = "";

		for (int i = 0; i < encryptedMessageBin.size() / 64; i++)
		{
			decryptedMessageBin += decryptBlock(encryptedMessageBin.substr(i * 64, 64), binKey);
		}

		return decryptedMessageBin;
	}
